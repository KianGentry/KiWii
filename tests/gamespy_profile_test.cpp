...
int main() {
	...
	assert(challenge.compare(challenge.size() - 12, 12, "\\id\\1\\final\\") ==
		   0);
	assert(challenge.size() == 38);
	const std::string login =
		"\\login\\\\challenge\\client\\authtoken\\NDStoken\\id\\1\\final\\";
	...
	assert(profile_response.find("\\nick\\KiWiiMTQyMDIzMjMyMDc0Ng%2A%2A\\") !=
		   std::string::npos);
	assert(
		profile_response.find("\\email\\KiWiiMTQyMDIzMjMyMDc0Ng%2A%2A@nds\\") !=
		std::string::npos);
	assert(profile_response.find(
			   "\\uniquenick\\KiWiiMTQyMDIzMjMyMDc0Ng%2A%2A\\") !=
		   std::string::npos);
	assert(profile_response.find(
			   "\\firstname\\Wii:first@RMCP\\lastname\\last-value\\") !=
		   std::string::npos);
	assert(profile_response.compare(profile_response.size() - 12, 12,
									"\\id\\2\\final\\") == 0);
	const std::string updatepro = "\\updatepro\\\\sesskey\\12345678\\firstname"
								  "\\Wii:RMCP\\partnerid\\11\\final\\";
	...
	const std::string login_response =
		mkwii::profile_login_response(login, "SERVERCHALL", credentials);
