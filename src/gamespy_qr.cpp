...
std::vector<std::uint8_t> rc4_transform(std::string_view key,
    ...
    for (std::size_t index = 0; index < state.size(); ++index) {
        swap_index = (swap_index + state[index] +
            static_cast<std::uint8_t>(key[index % key.size()])) & 0xff;
...
bool qr_challenge_matches(const std::vector<std::uint8_t>& packet,
    ...
    const std::vector<std::uint8_t> encrypted = rc4_transform(secret_key, plaintext);
    const std::string expected = base64_encode(encrypted);
    const auto response_begin = packet.begin() + 5;
    const auto response_end = packet.back() == 0x00 ? packet.end() - 1 : packet.end();
...
std::vector<std::uint8_t> qr_client_challenge_response(std::uint32_t session_id,
    ...
    std::vector<std::uint8_t> plaintext(challenge.begin(), challenge.end());
    plaintext.push_back(0x00);
    const std::string encoded = base64_encode(rc4_transform(secret_key, plaintext));
    std::vector<std::uint8_t> response = {
        0x01,
        static_cast<std::uint8_t>(session_id & 0xff),
        static_cast<std::uint8_t>((session_id >> 8) & 0xff),
